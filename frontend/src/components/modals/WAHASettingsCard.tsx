...
export default function WAHASettingsCard({ onToast }: WAHASettingsCardProps) {
  ...
  const [sessionName, setSessionName] = useState('hirepath')
  ...
  const fetchStatus = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (data.host) setHost(h => h || data.host)
      if (data.name) setSessionName(s => s || data.name)
  ...
  const handleSaveConfig = async () => {
    if (!host.trim()) { setError('Please enter the WhatsApp Gateway URL'); return }
    ...
    try {
      ...
      setConfigSaved(true)
      onToast?.('WhatsApp Gateway configuration saved!')
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-foreground flex items-center gap-2">
                ...
                <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-emerald-500/10 text-emerald-600 border border-emerald-500/20">Gateway</span>
              </h2>
              <p className="text-xs text-muted mt-0.5">Connect WhatsApp Integration Engine to send interview invitations & enable the AI WhatsApp agent</p>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="sm:col-span-2 space-y-1.5">
              <label className="text-xs font-semibold text-muted" htmlFor="waha-host">WhatsApp Gateway URL</label>
          ...
          <div className="flex items-center justify-between">
            ...
            {sessionStatus?.host && (
              <a href={`${sessionStatus.host}/dashboard`} target="_blank" rel="noopener noreferrer"
                ...
                Open Gateway Console <ExternalLink className="w-3 h-3" />
              </a>
            )}
          </div>
            ...
            <div className="flex items-center gap-3 p-4 rounded-2xl bg-rose-500/8 border border-rose-500/20 animate-fade-in">
              ...
              <div>
                <p className="text-sm font-bold text-rose-600">
                  {currentStatus === 'UNREACHABLE' ? 'Cannot Reach WhatsApp Gateway' : 'Session Failed'}
                </p>
