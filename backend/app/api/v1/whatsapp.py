...
class SendInviteRequest(BaseModel):
    ...
    employer_name: str = Field(default="HirePath Sri Lanka", example="WSO2 Lanka")
...
class SendOfferRequest(BaseModel):
    ...
    candidate_name: str = Field(..., example="Kasun Perera")
    job_title: str = Field(..., example="Senior React Developer")
    employer_name: str = Field(default="HirePath Client", example="WSO2 Lanka")
...
async def send_test_message(payload: WAHATestRequest, _: None = Depends(require_admin)):
    ...
    result = await waha_service.send_text(
        ...
        text=(
            "✅ *HirePath WAHA Test*\n\n"
            "This is a test message from the HirePath recruitment platform. "
...
class SendSlotsRequest(BaseModel):
    ...
    employer_name: str = Field(default="HirePath Client", example="TechCorp")
...
async def notify_new_job(payload: NewJobNotificationRequest):
    ...
    for phone in payload.phones:
        ...
        job_lines = "\n\n".join(
            f"💼 *{i+1}. {j.get('title')}*\n"
            f"🏢 {j.get('company', 'HirePath Client')}, {j.get('location', 'Colombo')}\n"
        ...
        msg = (
            f"🔔 *නව රැකියා දැන්වීම!* — HirePath Sri Lanka\n\n"
...
async def waha_webhook(request: Request):
    ...
    try:
        ...
        import logging
        logging.getLogger("hirepath.waha").info(f"Received WAHA webhook body: {body}")
    except Exception as e:
