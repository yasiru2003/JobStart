...
logger = logging.getLogger("hirepath.whatsapp_agent")
...
LANG_TEMPLATES = {
    "si": {
        "greeting":         "ආයුබෝවන්! 👋 HirePath වෙත සාදරයෙන් පිළිගනිමු.",
    ...
    "ta": {
        "greeting":         "வணக்கம்! 👋 HirePath-ல் உங்களை வரவேற்கிறோம்.",
    ...
    "en": {
        "greeting":         "Hello! 👋 Welcome to HirePath — Sri Lanka's AI Recruitment Platform.",
...
class WhatsAppAgentService:
    """
    Full-featured AI-powered WhatsApp recruitment agent for HirePath.
    ...
    def __init__(self):
        ...
        self.agent_name = "HirePath Recruitment Team"
            ...
            async with httpx.AsyncClient(timeout=12.0) as client:
                resp = await client.post(
                    ...
                    headers={
                        ...
                        "X-Title": "HirePath WhatsApp Agent",
            ...
            async with httpx.AsyncClient(timeout=6.0) as client:
                resp = await client.post(
                    ...
                    headers={
                        "Authorization": f"Bearer {self.api_key}",
                        "HTTP-Referer": "https://hirepath.lk",
                        "X-Title": "HirePath Intent Classifier",
                        "Content-Type": "application/json",
                    },
                    json={
                        "model": self.model,
    ...
    async def process_inbound_message(
        ...
        if has_unconfirmed_invite:
            ...
            if any(kw in lower_msg for kw in ("yes", "1", "confirm", "ow", "hari", "join", "sure", "ok", "yep", "agree", "thawuruyi")):
                ...
                meet_link = "https://meet.google.com/hirepath-interview-room"
        ...
        if intent == "SLOT_SELECT" and slot_index and available_slots:
            ...
            if 0 <= idx < len(available_slots):
                ...
                meet_link = f"https://meet.google.com/hirepath-{phone[-4:]}"
                reply = (
        ...
        if jobs_list:
            jobs_context = "\n".join(
                f"• [{i+1}] *{j.get('title')}* — {j.get('company', 'HirePath Client')} ({j.get('location', 'Sri Lanka')})\n"
        ...
        system_context_prompt = (
            f"=== HIREPATH SRI LANKA OFFICIAL AI RECRUITMENT DIRECTIVE ===\n"
            f"ROLE: You are the Lead AI Recruitment Executive for HirePath Sri Lanka (hirepath.lk).\n"
            f"CANDIDATE CONTEXT:\n"
            ...
            f"7. GREETINGS: If the user says 'Hi' or greets you for the first time, warmly welcome them in Sinhala (සිංහල) with a touch of Singlish, ask about their qualifications/interests, and proactively suggest 2-3 matching jobs from the LIVE DATABASE."
        )
    ...
    async def _route_intent(
        ...
        if intent == "HELP":
            ...
            system_prompt = (
                f"You are a professional WhatsApp recruitment assistant for HirePath Sri Lanka. "
    ...
    def _help_menu(self, lang: str = "si", available_jobs: Optional[List[Dict[str, Any]]] = None) -> str:
        ...
        if lang == "si":
            return (
                "👋 *ආයුබෝවන්! HirePath AI රැකියා සෙවුම* 🇱🇰\n\n"
            ...
            return (
                "👋 *Welcome to HirePath AI Job Finder!* 🇱🇰\n\n"
